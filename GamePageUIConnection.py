...
class GamePageUIConnection(object):
    ...
    def start_game_page_runner_and_lock(self):
        ...
        self.ui.gamepage.status_progress_bar.setValue(0)
    ...
    def finish_game_page_runner_and_open(self, info_str=''):
        ...
        self.ui.gamepage.status_progress_bar.setValue(100)
...
class GamePageRunner(QThread):
    ...
    def kirikiri_run(self):
        ...
        # 临时文件夹
        with tempfile.TemporaryDirectory() as kirikiri.tmp_folder:
            kirikiri.tmp_folder = Path(kirikiri.tmp_folder)
            # 给ui起个别名
            ugk = self.vnc.ui.gamepage.kirikiri
            if ugk.currentWidget() is ugk.hd_parts_frame:
                # 设置输入、输出路径
                kirikiri.set_game_data(self.vnc.input_folder, self.vnc.output_folder)
                # 设置原生分辨率和主要编码
    ...
    def artemis_run(self):
        ...
        # 临时文件夹
        with tempfile.TemporaryDirectory() as artemis.tmp_folder:
            artemis.tmp_folder = Path(artemis.tmp_folder)
            # 给ui起个别名
            uga = self.vnc.ui.gamepage.artemis
            if uga.currentWidget() is uga.hd_parts_frame:
                # 设置输入、输出路径
                artemis.set_game_data(self.vnc.input_folder, self.vnc.output_folder)
                # 设置原生分辨率和主要编码
