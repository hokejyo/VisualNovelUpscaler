...
class ImageUtils(object):
    ...
    def zoom_images(self, image_file_ls, zoom_factor):
        """
        @brief      将列表中的图片文件批量缩放，覆盖所有图片

        @param      image_file_ls  图片文件列表
        @param      zoom_factor    缩放系数
        """
        zoom_images_pool = Pool(self.cpu_cores)
        for image_file in image_file_ls:
            zoom_images_pool.apply_async(self.zoom_image, args=(image_file, zoom_factor))
        zoom_images_pool.close()
        zoom_images_pool.join()
    ...
    def convert_images(self, image_file_ls, output_extention, del_input=False) -> list:
        """
        @brief      批量转换列表中的图片文件格式

        @param      image_file_ls     输入图片路径列表
        @param      output_extention  输出图片格式
        @param      del_input         是否删除输入图片

        @return     转换后图片的路径对象列表
        """
        converted_image_path_ls = []
        convert_images_pool = Pool(self.cpu_cores)
        for image_file in image_file_ls:
            convert_images_p = convert_images_pool.apply_async(self.convert_image, args=(image_file, output_extention, del_input))
            converted_image_path_ls.append(convert_images_p.get())
        convert_images_pool.close()
        convert_images_pool.join()
        return converted_image_path_ls
    ...
    def ncnn_vulkan_series(self, input_path, output_folder, output_extention) -> list:
        """
        @brief      使用**-ncnn-vulkan系列超分辨率引擎放大图片
    ...
    def ncnn_vulkan_single(self, input_path, output_folder, output_extention) -> Path:
        """
        @brief      使用**-ncnn-vulkan系列超分辨率引擎放大单张图片
    ...
    def ncnn_vulkan_folder(self, input_path, output_folder, output_extention) -> list:
        """
        @brief      使用**-ncnn-vulkan系列超分辨率引擎放大单个文件夹中的图片，文件夹内图片格式需保持一致，目前的最佳选择，失真较小
        ...
        zoom_factor = self.scale_ratio/actiual_scale_ratio
        if zoom_factor != 1:
            self.zoom_images(tmp_image_ls, zoom_factor)
        # 转格式
        if output_extention != 'png':
            tmp_image_ls = self.convert_images(tmp_image_ls, output_extention, del_input=True)
        output_image_ls = []
